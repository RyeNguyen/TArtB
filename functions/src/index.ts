...
const refreshArtworks = async (): Promise<{ success: boolean; stats: any }> => {
  try {
    ...
    logger.info(
      `Found ${newArtworks.length} new artworks, ${duplicateCount} duplicates (skipped)`,
    );
    ...
    for (let i = 0; i < newArtworks.length; i += batchSize) {
      ...
      logger.info(
        `Written ${totalWritten}/${newArtworks.length} new artworks to Firestore`,
      );
    }
...
export const refreshArtworksHttp = onRequest(
  ...
  async (
    _req: any,
    res: {
      json: (arg0: { success: boolean; stats: any }) => void;
      status: (arg0: number) => {
        (): any;
        new (): any;
        json: {
...
export const migrateSquareToLandscape = onRequest(
  ...
  async (
    ...
    try {
      ...
      logger.info(
        `Found ${squareArtworks.size} artworks with square orientation`,
      );
